...
import { PointMarkerOptionsObject, Options } from 'highcharts';
import * as Maths from '../../utils/maths';

const statusToMarker = (status: number): PointMarkerOptionsObject => {
  const statusHead: number = Math.floor(status / 100);
  const createMarker = (color: string): PointMarkerOptionsObject => ({
    enabled: true,
    fillColor: color,
    lineColor: 'gray',
    lineWidth: 1,
  ...
  return statusHead === 5 ? createMarker('red') : statusHead === 4 ? createMarker('yellow') : { enabled: false };
};
