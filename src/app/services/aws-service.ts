...
function fetchObject<T>(s3: S3, bucket: string, objectKey: string): Promise<T> {
  return new Promise<T>((resolve, reject) => {
    s3.getObject({ Key: objectKey, Bucket: bucket }, (err, data) =>
      err ? reject(err.message) : resolve(JSON.parse(data.Body.toString())),
    );
  });
}
...
function putObject<T>(s3: S3, bucket: string, objectKey: string, body: any): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    s3.putObject({ Key: objectKey, Bucket: bucket, Body: body }, (err, data) =>
...
function deleteObjects(s3: S3, bucket: string, objectKeys: string[]): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    s3.deleteObjects({ Bucket: bucket, Delete: { Objects: objectKeys.map(k => ({ Key: k })) } }, (err, data) =>
      err ? reject(err.message) : resolve(),
    );
  });
}
...
export class AwsService {
  ...
  async pingBucket(): Promise<void | string> {
    ...
    return new Promise<void | string>((resolve, reject) => {
      this.s3.headBucket({ Bucket: this.bucket }, err =>
        err ? reject(err.code === 'NetworkingError' ? `Invalid bucket name` : 'Unexpected error') : resolve(),
      );
    });
  }
      ...
      this.s3.listObjectsV2(
        ...
        (err, data) =>
          err
            ...
            : data.KeyCount > 0
            ? resolve()
            : reject(`${this.bucket}/${this.prefix} is not existed or empty`),
      );
    });
  }
  ...
  async fetchFile(key: string, name: string): Promise<{ encoding: string; body: string }> {
    ...
    return new Promise<{ encoding: string; body: string }>((resolve, reject) => {
      this.s3.getObject({ Key: `${this.dataPrefix}/${key}/${name}`, Bucket: this.bucket }, (err, data) => {
        if (err) {
          return reject(err.message);
        } else {
          const encoding: string = Encoding.detect(data.Body);
          const body: string = Encoding.codeToString(Encoding.convert(data.Body, { from: encoding, to: 'UNICODE' }));
  ...
  async fetchArchive(key: string): Promise<{ name: string; body: Blob }> {
    ...
    return new Promise<{ name: string; body: Blob }>((resolve, reject) => {
      this.s3.getObject({ Key: `${this.dataPrefix}/${key}/${zipName}`, Bucket: this.bucket }, (err, data) =>
        err
          ? reject(err.message)
          : resolve({
              name: zipName,
  ...
  async findSummary(keyWord: string): Promise<DynamoResult> {
    ...
    const keys = ['hashkey', 'title', 'description', 'one_host', 'other_host', 'begin_time', 'check_status', 'tags'];
